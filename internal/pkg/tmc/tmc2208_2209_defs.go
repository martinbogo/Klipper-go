...
const TMC2208TMCFrequency = 12000000.
const TMC2209TMCFrequency = 12000000.

var TMC2208Registers = map[string]int64{
...
var TMC2208SignedFields = []string{"cur_a", "cur_b", "pwm_scale_auto"}

var TMC2208FieldFormatters = map[string]func(interface{}) string{
...
var TMC2209Registers = map[string]int64{
	"GCONF": 0x00, "GSTAT": 0x01, "IFCNT": 0x02, "SLAVECONF": 0x03,
	"OTP_PROG": 0x04, "OTP_READ": 0x05, "IOIN": 0x06, "FACTORY_CONF": 0x07,
	"IHOLD_IRUN": 0x10, "TPOWERDOWN": 0x11, "TSTEP": 0x12, "TPWMTHRS": 0x13,
	"VACTUAL": 0x22, "MSCNT": 0x6a, "MSCURACT": 0x6b, "CHOPCONF": 0x6c,
...
var TMC2209ReadRegisters = []string{
	"GCONF", "GSTAT", "IFCNT", "OTP_READ", "IOIN", "FACTORY_CONF", "TSTEP",
	"MSCNT", "MSCURACT", "CHOPCONF", "DRV_STATUS",
	"PWMCONF", "PWM_SCALE", "PWM_AUTO",
	"SG_RESULT",
...
var TMC2209Fields = map[string]map[string]int64{
	"GCONF":        TMC2208Fields["GCONF"],
	"GSTAT":        TMC2208Fields["GSTAT"],
	"IFCNT":        TMC2208Fields["IFCNT"],
	"SLAVECONF":    TMC2208Fields["SLAVECONF"],
	"OTP_PROG":     TMC2208Fields["OTP_PROG"],
	"OTP_READ":     TMC2208Fields["OTP_READ"],
	"IOIN@TMC222x": TMC2208Fields["IOIN@TMC222x"],
	"IOIN@TMC220x": TMC2208Fields["IOIN@TMC220x"],
	"FACTORY_CONF": TMC2208Fields["FACTORY_CONF"],
	"IHOLD_IRUN":   TMC2208Fields["IHOLD_IRUN"],
	"TPOWERDOWN":   TMC2208Fields["TPOWERDOWN"],
	"TSTEP":        TMC2208Fields["TSTEP"],
	"TPWMTHRS":     TMC2208Fields["TPWMTHRS"],
	"VACTUAL":      TMC2208Fields["VACTUAL"],
	"MSCNT":        TMC2208Fields["MSCNT"],
	"MSCURACT":     TMC2208Fields["MSCURACT"],
	"CHOPCONF":     TMC2208Fields["CHOPCONF"],
	"DRV_STATUS":   TMC2208Fields["DRV_STATUS"],
	"PWMCONF":      TMC2208Fields["PWMCONF"],
	"PWM_SCALE":    TMC2208Fields["PWM_SCALE"],
	"PWM_AUTO":     TMC2208Fields["PWM_AUTO"],
	"COOLCONF": {
		"semin":  0x0F << 0,
		"seup":   0x03 << 5,
		"semax":  0x0F << 8,
...
var TMC2209FieldFormatters = TMC2208FieldFormatters

func ConfigureTMC2208(config ConfigFieldSource, fields *FieldHelper) {
	...
	setConfigField(config, "hend", 0)
	setConfigField(config, "tbl", 2)
	setConfigField(config, "iholddelay", 8)
	setConfigField(config, "pwm_ofs", 36)
	setConfigField(config, "pwm_grad", 14)
	setConfigField(config, "pwm_freq", 1)
	setConfigField(config, "pwm_autoscale", true)
	setConfigField(config, "pwm_autograd", true)
	setConfigField(config, "pwm_reg", 8)
	setConfigField(config, "pwm_lim", 12)
	setConfigField(config, "tpowerdown", 20)
}

func ConfigureTMC2209(config ConfigFieldSource, fields *FieldHelper) {
	fields.Set_field("pdn_disable", true, nil, nil)
	...
	setConfigField(config, "pwm_lim", 12)
	setConfigField(config, "tpowerdown", 20)
	setConfigField(config, "sgthrs", 0)
}
