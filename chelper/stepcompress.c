...
struct stepcompress {
    ...
    struct list_head local_msg_queue;
    int owns_local_msg_queue;
    uint32_t oid;
...
stepcompress_alloc(struct list_head *msg_queue)
{
    ...
    if (msg_queue) {
        ...
        sc->owns_local_msg_queue = 0;
    } else {
        list_init(&sc->local_msg_queue);
        sc->msg_queue = &sc->local_msg_queue;
        sc->owns_local_msg_queue = 1;
    }
    return sc;
}

// Backward-compatible allocator used by existing Go bindings.
struct stepcompress * __visible
stepcompress_alloc_compat(uint32_t oid)
{
